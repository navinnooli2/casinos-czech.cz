...
<head>
  ...
  <style>
    ...
    th{background:#1a1a2e;color:#fff;padding:14px 16px;text-align:left;font-size:.78rem;text-transform:uppercase;letter-spacing:.5px}
    td{padding:11px 16px;border-bottom:1px solid #f0f0f0;font-size:.85rem}
    ...
    .badge{display:inline-block;padding:3px 10px;border-radius:4px;font-size:.7rem;font-weight:700;text-transform:uppercase;background:#e0e7ff;color:#4338ca;margin-right:8px}
  </style>
</head>
<body>
  <div class="header">
    <h1>Casinos Czech — Sitemap</h1>
    <p>Mapa stránek pro vyhledávače (Google, Bing, Seznam)</p>
  </div>
  <div class="container">
    <xsl:choose>
      <xsl:when test="sitemap:sitemapindex">
        <div class="count">
          Toto je <strong>sitemap index</strong> — obsahuje <strong><xsl:value-of select="count(sitemap:sitemapindex/sitemap:sitemap)"/></strong> dílčích sitemap souborů.
        </div>
        <table>
          <thead>
            <tr>
  ...
  <div class="footer">Casinos Czech .CZ — Sitemap generována automaticky</div>
</body>
</html>
</xsl:template>
</xsl:stylesheet>